...
/* ------------------------------------------------------------------ */
/*  SWAPPABLE TOKENS — the ONLY three customizable values             */
/* ------------------------------------------------------------------ */
const BUSINESS_NAME = "Apex Concrete & Co.";
const CITY = "Austin";
...
const TITLE = `Concrete Contractor in ${CITY} | ${BUSINESS_NAME}`;
const DESCRIPTION = `${BUSINESS_NAME} is ${CITY}'s licensed concrete contractor for driveways, patios, stamped concrete, foundations and repair. Free written estimates — call ${PHONE}.`;
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          address: {
            "@type": "PostalAddress",
            streetAddress: "1428 Industrial Blvd",
          ...
          areaServed: ["Round Rock", "Cedar Park", "Pflugerville", "Leander", "Buda", "Kyle", "Lakeway"].map(
            (n) => ({ "@type": "City", name: n }),
          ),
...
const FAQS = [
  ...
  { q: "Are you licensed and insured?", a: `Yes. ${BUSINESS_NAME} is fully licensed, bonded and carries general liability and workers' compensation insurance. License and insurance certificates are available on request before any work begins.` },
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 lg:h-20">
        <a href="#top" className="font-display text-2xl tracking-wide uppercase text-ink">
          {BUSINESS_NAME}
        </a>

        <nav aria-label="Primary" className="hidden lg:flex items-center gap-8">
          {navLinks.map((l) => (
            <a key={l.href} href={l.href} className="text-sm font-medium text-slate-concrete hover:text-ink transition-colors">
              {l.label}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <a
            href={TEL_HREF}
            className="hidden sm:inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-ink hover:text-primary-hover transition-colors"
          ...
          <a
            href="#contact"
            className="hidden sm:inline-flex items-center gap-2 px-4 py-2.5 rounded-md bg-primary text-primary-foreground font-semibold hover:bg-primary-hover transition-colors"
          >
            Free Estimate
          </a>
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="lg:hidden inline-flex items-center justify-center p-2 rounded-md text-ink"
          ...
          <nav aria-label="Mobile" className="px-4 py-3 flex flex-col">
            {navLinks.map((l) => (
              <a
                ...
                className="py-3 text-base font-medium text-ink border-b border-border last:border-0"
              >
                {l.label}
              </a>
            ))}
            <a
              href="#contact"
              onClick={() => setOpen(false)}
              className="mt-3 inline-flex items-center justify-center px-4 py-3 rounded-md bg-primary text-primary-foreground font-semibold"
            >
              Get Free Estimate
            </a>
          </nav>
...
function Hero() {
  return (
    <section id="top" className="relative isolate overflow-hidden bg-ink">
      <img
        src={heroImg}
        alt={`Premium concrete driveway poured by ${BUSINESS_NAME} in ${CITY}`}
        width={1920}
        height={1280}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-24 sm:py-32 lg:py-40">
        <div className="max-w-3xl">
          <p className="text-primary font-semibold uppercase tracking-[0.2em] text-xs sm:text-sm mb-5">
            Concrete Contractor — {CITY}, TX
          </p>
          <h1 className="text-white font-display text-5xl sm:text-6xl lg:text-7xl xl:text-8xl leading-[0.95]">
            {CITY}'s Trusted<br />Concrete Contractor
          </h1>
          <p className="mt-6 text-lg sm:text-xl text-white/85 max-w-2xl">
            Durable, expertly-poured driveways, patios, foundations and stamped concrete — backed by a written
            workmanship guarantee. Free on-site estimates across the {CITY} metro.
          </p>

          <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
            <a
              href="#contact"
              className="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-md bg-primary text-primary-foreground font-semibold text-base hover:bg-primary-hover transition-colors"
            >
            ...
            <a
              href={TEL_HREF}
              className="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-md border-2 border-white/30 text-white font-semibold text-base hover:bg-white/10 transition-colors"
            >
              <Phone size={18} strokeWidth={2} />
              Call {PHONE}
            </a>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-white/85">
            ...
            <span className="inline-flex items-center gap-2">
              <Award size={18} strokeWidth={2} className="text-primary" />
              22+ Years in {CITY}
            </span>
            <span className="inline-flex items-center gap-1.5">
              {[0,1,2,3,4].map((i) => (
                <Star key={i} size={18} strokeWidth={2} className="text-primary fill-[oklch(0.78_0.16_70)]" />
              ))}
    ...
    <section id="services" className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mb-12">
          ...
          <p className="mt-5 text-lg text-slate-concrete">
            From a single sidewalk repair to a full commercial pad, every job is engineered, poured and finished by
            our in-house {CITY} crews. No subcontractors. No surprises.
          </p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {SERVICES.map((s) => (
            <article
              key={s.name}
              className="group bg-card rounded-lg overflow-hidden border border-border shadow-card hover:shadow-card-hover transition-shadow"
            >
              <div className="relative aspect-[4/3] overflow-hidden bg-concrete">
                <img
                  src={s.img}
                  alt={`${s.name} project completed by ${BUSINESS_NAME} in ${CITY}`}
                  width={1024}
                  height={768}
                  loading="lazy"
              ...
              <div className="p-5">
                <div className="flex items-center gap-3 mb-3">
                  <span className="inline-flex items-center justify-center w-10 h-10 rounded-md bg-ink text-primary shrink-0">
                ...
                <a
                  href="#contact"
                  className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-ink hover:text-primary-hover"
                >
                  Get a quote
...
function WhyUs() {
  const items = [
    ...
    { icon: Hammer, h: "Skilled In-House Crews", p: "Every pour, every finish, every cut — done by Apex employees. We never subcontract your project to the lowest bidder." },
    ...
    <section className="bg-concrete py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div>
          ...
          <h2 className="text-4xl sm:text-5xl text-ink">Why {CITY} homeowners pick {BUSINESS_NAME}</h2>
          <p className="mt-5 text-lg text-slate-concrete">
            We've poured concrete across the {CITY} metro for more than two decades. Here's what you get when you
            hire us — and what most contractors won't put in writing.
          </p>
          <div className="mt-8 grid sm:grid-cols-2 gap-6">
            {items.map((it) => (
              <div key={it.h}>
                <span className="inline-flex items-center justify-center w-12 h-12 rounded-md bg-ink text-primary mb-3">
                  <it.icon size={28} strokeWidth={1.75} />
                </span>
        ...
        <div className="relative">
          <img
            src={crewImg}
            alt={`${BUSINESS_NAME} crew screeding a freshly poured foundation slab in ${CITY}`}
            width={1024}
            height={768}
            loading="lazy"
            className="rounded-lg w-full h-auto object-cover shadow-card-hover"
          />
    ...
    <section id="areas" className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-start">
        <div>
          ...
          <p className="mt-5 text-lg text-slate-concrete">
            Based in {CITY}, we pour concrete across the metro and into the suburbs. If your town isn't listed,
            call us — we likely still serve you.
          </p>
        ...
        <div className="rounded-lg overflow-hidden border border-border bg-concrete aspect-[4/3] lg:aspect-auto lg:h-[460px]">
          <iframe
            title={`Map of ${BUSINESS_NAME} service area in ${CITY}`}
            src="https://www.openstreetmap.org/export/embed.html?bbox=-97.95%2C30.15%2C-97.55%2C30.45&amp;layer=mapnik"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
        <div className="max-w-3xl mb-12">
          ...
          <div className="mt-5 flex items-center gap-3 text-white/85">
            <div className="flex">
              {[0,1,2,3,4].map((i) => (
                <Star key={i} size={20} strokeWidth={2} className="text-primary fill-[oklch(0.78_0.16_70)]" />
              ))}
            </div>
            <span className="text-sm">
              <strong className="text-white">4.9</strong> from <strong className="text-white">187</strong> reviews —{" "}
              <a href="#" className="underline underline-offset-4 hover:text-primary">see Google reviews</a>
            </span>
          </div>
        </div>
            ...
            <article key={r.name} className="bg-white/[0.04] border border-white/10 rounded-lg p-6">
              ...
              <div className="mt-5 pt-5 border-t border-white/10 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-white/60">{r.area}</p>
                </div>
                <div className="flex">
                  {[0,1,2,3,4].map((i) => (
                    <Star key={i} size={14} strokeWidth={2} className="text-primary fill-[oklch(0.78_0.16_70)]" />
                  ))}
                </div>
              </div>
            </article>
              ...
              <div key={f.q}>
                ...
                >
                  ...
                  <ChevronDown
                    size={20}
                    strokeWidth={2}
                    className={`text-slate-concrete shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isOpen && (
                  <div className="pb-6 pr-10 text-slate-concrete">{f.a}</div>
                )}
              </div>
            );
          })}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
        <div className="max-w-2xl">
          <h2 className="text-4xl sm:text-5xl text-white">
            Get Your Free Concrete Estimate in {CITY} Today
          </h2>
          <p className="mt-4 text-white/80 text-lg">
            On-site measurement, written quote inside 48 hours, zero obligation.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          <a
            href="#contact"
            className="inline-flex items-center justify-center px-6 py-4 rounded-md bg-primary text-primary-foreground font-semibold hover:bg-primary-hover transition-colors"
          >
            Request Free Estimate
          </a>
          <a
            href={TEL_HREF}
            className="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-md border-2 border-white/30 text-white font-semibold hover:bg-white/10 transition-colors"
          >
            <Phone size={18} strokeWidth={2} />
            Call {PHONE}
          </a>
...
function Contact() {
  const [form, setForm] = useState({
    name: "", phone: "", email: "", service: SERVICES[0].name, details: "", zip: "",
  });
    ...
    <section id="contact" className="bg-concrete py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-5 gap-10 lg:gap-16">
        <div className="lg:col-span-3">
          ...
          <p className="mt-4 text-lg text-slate-concrete">
            Fill this out and we'll be in touch within one business day to schedule your free on-site estimate.
          </p>
          ...
          {submitted ? (
            <div className="mt-8 p-8 bg-card border border-success/40 rounded-lg flex gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-slate-concrete">
                  Thanks, {form.name}. We'll call you at {form.phone} within one business day to schedule your
                  free on-site estimate. Need to talk now? Call us at{" "}
                  <a href={TEL_HREF} className="text-primary-hover font-semibold underline">{PHONE}</a>.
                </p>
              </div>
            </div>
          ) : (
            <div className="mt-8 bg-card border border-border rounded-lg p-6 sm:p-8 grid sm:grid-cols-2 gap-5">
              <Field label="Full name" id="name" error={errors.name}>
                <input id="name" value={form.name} onChange={(e) => update("name", e.target.value)} className={inputCls} />
              </Field>
              ...
              <Field label="Email" id="email" error={errors.email}>
                <input id="email" type="email" value={form.email} onChange={(e) => update("email", e.target.value)} className={inputCls} />
              </Field>
              <Field label="ZIP or city" id="zip" error={errors.zip}>
                <input id="zip" value={form.zip} onChange={(e) => update("zip", e.target.value)} className={inputCls} />
              </Field>
              ...
              <div className="sm:col-span-2">
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-4 rounded-md bg-primary text-primary-foreground font-semibold hover:bg-primary-hover transition-colors"
                >
                  Get My Free Estimate
                  <ArrowRight size={18} strokeWidth={2} />
                </button>
                <p className="mt-3 text-xs text-slate-concrete">
        ...
        <aside className="lg:col-span-2">
          <div className="bg-ink concrete-grain text-white rounded-lg p-6 sm:p-8 relative">
            <h3 className="font-display text-2xl uppercase">{BUSINESS_NAME}</h3>
            <p className="mt-2 text-white/70 text-sm">{CITY}'s trusted concrete contractor.</p>

            <ul className="mt-6 space-y-4 text-sm">
              <li className="flex items-start gap-3">
                <Phone size={18} strokeWidth={2} className="text-primary mt-0.5 shrink-0" />
                <a href={TEL_HREF} className="hover:text-primary">{PHONE}</a>
              </li>
              <li className="flex items-start gap-3">
                <Mail size={18} strokeWidth={2} className="text-primary mt-0.5 shrink-0" />
                <a href="mailto:estimates@apexconcreteco.com" className="hover:text-primary">estimates@apexconcreteco.com</a>
              </li>
              <li className="flex items-start gap-3">
                <MapPin size={18} strokeWidth={2} className="text-primary mt-0.5 shrink-0" />
                <span>1428 Industrial Blvd<br />{CITY}, TX 78702</span>
              </li>
              <li className="flex items-start gap-3">
                <Clock size={18} strokeWidth={2} className="text-primary mt-0.5 shrink-0" />
                <span>Mon–Fri 7:00a–6:00p<br />Sat 8:00a–2:00p<br />Sun closed</span>
              </li>
            </ul>
          </div>
        </aside>
...
const inputCls =
  "w-full px-4 py-3 rounded-md bg-background border border-border text-ink placeholder:text-slate-concrete/60 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary";
  ...
  return (
    <footer className="bg-ink concrete-grain text-white/80">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <p className="font-display text-2xl uppercase text-white">{BUSINESS_NAME}</p>
          <p className="mt-3 text-sm">
            {CITY}'s trusted concrete contractor for driveways, patios, foundations and decorative concrete.
          </p>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li><a href="#areas" className="hover:text-primary">Service Areas</a></li>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {SERVICES.slice(0, 6).map((s) => (
              <li key={s.name}><a href="#services" className="hover:text-primary">{s.name}</a></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-sm">
            <li><a href={TEL_HREF} className="hover:text-primary">{PHONE}</a></li>
...
function StickyCallBar() {
  return (
    <a
      href={TEL_HREF}
      className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-primary text-primary-foreground font-semibold py-4 flex items-center justify-center gap-2 shadow-card-hover"
      aria-label={`Call ${BUSINESS_NAME} at ${PHONE}`}
    >
